...
export function WorkspaceDashboardClient({ slug }: WorkspaceDashboardClientProps) {
  const { data: session } = useSession()
  const { data: workspaceData, isLoading: isLoadingWorkspace } = useWorkspace(slug)
  const workspace = workspaceData?.workspace

  const { data: projectsData, isLoading: isLoadingProjects } = useProjects(slug, {
    enabled: !!workspace
  })
  ...
  const { data: tasksData } = useTasks({
    ...
    enabled: !!workspace && projects.length > 0
  })
  const recentTasks = (tasksData?.tasks || []).slice(0, 8)

  const { data: milestonesData } = useMilestones(workspace?.id || "", {
    enabled: !!workspace && projects.length > 0
  })
  const upcomingMilestones = (milestonesData?.milestones || [])
    .filter((m: any) => m.endDate && new Date(m.endDate) >= new Date())
    .slice(0, 5)

  const { members = [] } = useWorkspaceMembers({
    workspaceId: slug, // Use slug, API will resolve to ID
  })
  ...
  const productivityScore = Math.round(
    (completedTasks * 0.4 + inProgressTasks * 0.3 + (totalTasks - blockedTasks) * 0.3)
  )
  ...
  const projectProgressData = projects.map((project) => ({
    ...
    progress: Math.round(
      ((project.tasks?.filter((t) => t.status === "done").length || 0) /
        Math.max(project._count.tasks, 1)) *
        100
    ),
  ...
  const teamPerformanceData = (Array.isArray(members) ? members : []).slice(0, 8).map((member: any) => ({
      ...
      <div className="p-4 space-y-4">
            ...
            <CardContent className="p-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-foreground">{Array.isArray(members) ? members.length : 0}</p>
                </div>
                    ...
                    <div key={milestone.id} className="flex items-center justify-between py-1">
                      <div className="flex items-center space-x-2 min-w-0 flex-1">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-xs text-gray-500">{milestone.project?.name}</p>
                        </div>
                      </div>
              ...
              <div className="space-y-2">
                {projects.length === 0 ? (
                  ...
                  projects.slice(0, 5).map((project) => {
                    ...
                    return (
                      <Link key={project.id} href={`/workspace/${slug}/projects/${project.id}`} className="block">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <Card>
            ...
            <CardContent className="pt-0">
                <div className="space-y-2">
                {(Array.isArray(members) ? members : []).slice(0, 8).map((member: any) => (
